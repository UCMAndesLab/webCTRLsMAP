...
def readJSON(jsonLocation):
    fileList = glob.glob("{0}/*.json".format(jsonLocation))
...
if __name__ == '__main__':
    ...
    parser.add_argument("jsonFile", help="Path to json file containing bacnet points")
    args = parser.parse_args()
    
    print 'Reading', args.csvFile
    locationDict = readCSV(args.csvFile)
    deviceDict = readJSON(args.jsonFile)
...
#             continue
        ...
        for (name, devID, objName) in locationDict[location]:
            ...
            if (devID) in deviceDict:
                ...
                if foundObj:
                    ...
                    foundObj['desc'] = location
                    while name in foundObjNames:
                      name = name + '_' + str(foundObj['props']['instance'])
                    foundObj['name'] = name
                    foundObjNames.add(name)
                ...
                print 'Found missing object', location +','+ name +','+ devID +','+ objName
                missingObjects.append(location +','+ name +','+ devID +','+ objName)
        ...
        for (devID,dev) in foundDevDict.items():
            allFoundList.append(dev)

    jsonStr = json.dumps(allFoundList, indent = 2);
    if args.csvFile.endswith('.csv'):
        fn = args.csvFile[: -3] + 'json'
    else:
        fn = args.csvFile + 'json'
